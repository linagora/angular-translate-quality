...
function verify_i18n_one(options, content, filename) {
  ...
  content.split('\n').forEach(function(val) {
    ...
    // Values cannot contain tabs, see http://stackoverflow.com/a/19799355/3594403
    if (value.match(/^\s*.+\t.+\s*$/)) {
      failure = output_i18n_error(
          options.cb,
          'Values cannot contain tabs (this is invalid json, escape them using \t). Key: ' + key, lineCpt);
    }
