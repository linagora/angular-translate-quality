...
/**
 * A callback invoked to deal with errors related to external keys.
 * @callback NotFoundKeysHandler
 * @param {ErrorHandler} cb An error handler.
 * @param {string[]} notFoundKeys Keys that were not found in HTML files.
 */


 /**
  * A callback invoked to deal with errors related to forbidden patterns.
  * @callback ForbiddenPatternsHandler
  * @param {ErrorHandler} cb An error handler.
  * @param {string} the translation key.
  * @param {string} the translation value.
  * @param {string} the error message.
  * @param {int} the error's line.
 ...
 * @property {boolean} ignore_order True to not verify that keys are sorted alphabetically.
 * @property {boolean} ignore_empty_values True to ignore empty values, false otherwise.
 * @property {boolean} ignore_empty_values True to fail even if only warnings were found.
 * @property {string} indent The indentation character used in JSon files.
 * @property {string[]} exclusions An array of (entire) strings that should be considered as not translatable.
 * @property {ErrorHandler} cb An error handler.
 * @property {NotFoundKeysHandler} external_keys_cb A handler for keys that were not found.
 * @property {ForbiddenPatternsHandler} forbidden_patterns_cb A handler for values that contain forbidden patterns.
...
function fix_options(options) {
  ...
  if (typeof(options.ignore_order) !== 'boolean') {
    options.ignore_order = false;
  }

  if (typeof(options.ignore_empty_values) !== 'boolean') {
    options.ignore_empty_values = false;
  }
...
function verify_i18n_one(options, content, filename) {
  ...
  content.split('\n').forEach(function(val) {
    ...
    if (! val.match(regex)) {
      failure = output_i18n_error(options.cb, 'Lines must match the following pattern: ' + options.indent + '"key": "value". ' + val, lineCpt);
      return;
    }
    ...
    if (value !== value.trim()) {
      failure = output_i18n_error(
          options.cb,
          'Values should be trimmed (no white space character at the beginning and the end). Key: ' + key, lineCpt);
    }

    // Values should not contain tabs, see http://stackoverflow.com/a/19799355/3594403
    if (value.match(/^\s*.+\t.+\s*$/)) {
      failure = output_i18n_error(
          options.cb,
          'Values should not contain tabs (this is invalid json, escape them using \t). Key: ' + key, lineCpt);
    }

    if (!options.ignore_empty_values && value === '') {
      failure = output_i18n_error(
          options.cb,
          'Values should not be empty. Key: ' + key, lineCpt);
    }
    ...
    if (!options.ignore_order &&
          keyLength > 1 &&
          0 > key.toLowerCase().localeCompare(keyArray[keyLength - 2].toLowerCase())) {
    ...
    if (options.forbidden_patterns &&
      ...
      options.forbidden_patterns[filename].forEach(function(val) {
        ...
        if (value.match(regexp)) {
          if (options.forbidden_patterns_cb) {
            var considerAsError = options.forbidden_patterns_cb(options.cb, key, value, val.msg, lineCpt, filename);
            failure = failure || considerAsError;
          } else {
