...
function internal_validate(options) {
  ...
  if (options.check_html && verify_non_translated_text(options)) {
...
function verify_non_translated_text(options) {
  ...
  glob.sync(options.loc_html + '*.html').forEach(function(val) {
    ...
    var m;
    var pattern = /\balt\s*=\s*"([^"]*)"/g;
    while ((m = pattern.exec(content))) {
      if (! m[1].match(/\{\{.*\}\}/g)) {
        failure = output_i18n_error(options.cb, '[ WARNING ] Non-translated text in ' + filename + ': "' + m[1] + '"');
        failure = true;
      }
    }
  });
