...
function internal_validate(options) {
  ...
  glob.sync(options.loc_i18n + '*.json').forEach(function(val) {
      ...
      var res = verify_i18n_one(options, content, filename.replace('.json', ''));
      if (res.failure) {
  ...
  glob.sync(options.loc_html + '*.html').forEach(function(val) {
    ...
    for (var i = 0; i < attributes.length; i++) {
      ...
      while ((m = pattern.exec(content))) {
        ...
        var attValue = m[2].replace(/&\S+;/, '').trim();
        if (attValue.length > 0 && ! attValue.match(/^\{\{.*\}\}$/)) {
    ...
    while ((m = pattern.exec(content))) {
      ...
      // Replace HTML entities and ignore empty texts
      var markupContent = m[1].replace(/&\S+;/, '').trim();
      if (markupContent.length !== 0 && ! markupContent.match(/^\{\{.*\}\}$/)) {
        markupContent = markupContent.replace(/\s+/g, ' ');
    ...
    pattern = new RegExp('\\{\\{.*\'(.*)\'(.*)\\}\\}', 'g');
    while ((m = pattern.exec(content))) {
      if (m[2].indexOf('translate') === -1) {
        failure = true;
        failure = output_i18n_error(
            options.cb,
            '[ WARNING ] Non-translated text might have been forgotten in ' + filename + ': "' + m[1].trim() + '"');
      }
    }
  });
